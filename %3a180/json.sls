(library (srfi :180 json)
  ...
  (import (rnrs (6))
          ...
          (srfi :180 helpers)
          (only (srfi :151 bitwise-operations) arithmetic-shift bitwise-ior))
  ...
  (define (port->generator port)
    (let ((count 0)
      ...
      (lambda ()
        (let ((out (guard (ex
                           (else
                            (raise (make-json-error (format "Read error at ~s." count)))))
                     (read-char port))))
          (if (= count limit)
  ...
  (define (%json-tokens generator)
    ...
    (define (read-unicode-escape generator)
      (let* ((one (maybe-char generator))
             ...
             (code-point (list one two three four)))
        (let ((out (string->number (list->string code-point) 16)))
          (if out
              out
              (raise (make-json-error (format "Invalid code point ~s." code-point)))))))
            ...
            (begin
              (let loop-unescape ((char (generator))
                ...
                (case char
                  ...
                  (else (raise (make-json-error (format "Unexpected escaped sequence ~s." chars-unescaped)))))))
            (cond
             ((char=? char #\")
            ...
            (let ((string (list->string (reverse out))))
              (if (valid-number? string)
                  (let ((number (string->number string)))
                    (if number
                        (values number char)
                        (raise (make-json-error (format "Invalid number ~s." string)))))
                  (raise (make-json-error (format  "Invalid number ~s." string)))))
            (loop (generator) (cons char out)))))
        ...
        (let ((token (tokens)))
          (case token
            ...
            ((comma) (let ((token (tokens)))
                       (unless (string? token)
                         (raise (make-json-error (format "Invalid object ~s, expected an object key" token))))
                       (values token
                               (object-colon tokens k))))
            (else (raise (make-json-error (format "Invalid object ~s, expected comma or object close." token))))))))
      ...
      (let ((token (tokens)))
        (if (eof-object? token)
            ...
            (cond
             ...
             (else (raise (make-json-error (format "Is it JSON text?! ~s" token))))))))
  ...
  (define (json-accumulator accumulator)
    ...
    (define (write-json-value obj accumulator)
      (cond
       ...
       (else (raise (make-json-error (format "Invalid json value ~s." obj))))))
    ...
    (define (raise-unless-valid? obj)
      (cond
       ...
       ((pair? obj)
        (for-each (lambda (obj)
                    (unless (pair? obj)
                      (raise (make-json-error (format "Unexpected object ~s, not a pair." obj))))
                    (unless (symbol? (car obj))
                      (raise (make-json-error (format "Unexpected object ~s, not a symbol key." obj))))
                    (raise-unless-valid? (cdr obj)))
                  obj))
       (else (raise (make-json-error (format "Unexpected object ~s" obj))))))
      ...
      (cond
       ...
       ((pair? obj)
        ...
        (for-each (lambda (pair)
                  ...
                  obj)
        (accumulator '(json-structure . object-end)))
       (else (error 'json-write "Unexpected error!" obj))))
  ...
  (define (port->accumulator port)
    (lambda (char-or-string)
      (cond
       ...
       ((string? char-or-string) (display char-or-string port))
